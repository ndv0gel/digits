...
interface AddContactFormProps { // DEFINE PROPS FOR THE COMPONENT
  userEmail: string;
}

const AddContactForm: React.FC<AddContactFormProps> = ({ userEmail }) => { // ACCEPT userEmail PROP
  ...
  const onSubmit = async (data: AddContactFormInputs) => {
    if (!userEmail) { // CHECK userEmail PROP
      console.error('User email not provided to AddContactForm.');
      router.push('/auth/signin'); // Redirect if no email
      return;
    }
    await addContact({ ...data, owner: userEmail }); // USE userEmail PROP
    reset();
    router.push('/list'); // Redirect to list page after adding
  };

  return ( // <-- ENSURE THIS OPENING PARENTHESIS IS HERE
  ...
  ); // <-- AND THIS CLOSING PARENTHESIS IS HERE
};
